...
def multiple_register_read(_input_or_holding, _address, _length, _data_type, _big_endian = False):
    ...
    if(_data_type == "INT64"):
        ...
        if data:
            while(len(data) >= 4):
                this_INT64 = (data[0:4][3] << 16*3) + (data[0:4][2] << 16*2) + (data[0:4][1] << 16*1) + data[0:4][0]
...
def Read_PM2100(client_obj, RS485_address):
    ...
    try:
        ...
        f.write("Error in Read Data From PM2100")
...
def read_onBoard_sensors():
    client.unit_id(13)
    a = multiple_register_read("input", 1000, 1, "INT16")
    if (a):
        if (a[0]):
            # print(a[0])
            return a[0]
...
client = ModbusClient(host = "192.168.1.239", port = 502, auto_open = True, auto_close = True, timeout = 0.1, debug = False)
...
while(1):
    sec, min, hour = read_on_timer()
    temp = read_temperature()

    log_str0 = "sec = " + str(round(sec, 1)) + " | " + "min = " + str(round(min, 1)) + " | " + "hour = " + str(round(hour, 1)) + " | Temp = " + str(temp) + '\n'

    print(log_str0, end  = '')
    f.write(log_str0)

    if (int(sec) % 5 == 0 and int(sec) != last_hit_sec):
        last_hit_sec = int(sec)
        rs_read_cnt = rs_read_cnt + 1

        log_str1 = "RS485 Read Counter : " + str(rs_read_cnt) + '\n'
        print(log_str1, end  = '')
        f.write(log_str1)
